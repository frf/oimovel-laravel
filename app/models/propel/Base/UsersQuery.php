 ...
 *
 *
 * @method     ChildUsersQuery orderByTimeCreated($order = Criteria::ASC) Order by the time_created column
 ...
 *
 * @method     ChildUsersQuery groupByTimeCreated() Group by the time_created column
 ...
 *
 * @method     ChildUsersQuery leftJoinUserCustomFields($relationAlias = null) Adds a LEFT JOIN clause to the query using the UserCustomFields relation
 * @method     ChildUsersQuery rightJoinUserCustomFields($relationAlias = null) Adds a RIGHT JOIN clause to the query using the UserCustomFields relation
 * @method     ChildUsersQuery innerJoinUserCustomFields($relationAlias = null) Adds a INNER JOIN clause to the query using the UserCustomFields relation
 *
 * @method     \UserCustomFieldsQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 *
 * @method     ChildUsers findOneByTimeCreated(int $time_created) Return the first ChildUsers filtered by the time_created column
 ...
 * @method     ChildUsers[]|ObjectCollection findByTimeCreated(int $time_created) Return ChildUsers objects filtered by the time_created column
...
abstract class UsersQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'oimovel', $modelName = '\\Users', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT time_created, name, roles, salt, password, email, id FROM users WHERE id = :p0';
        try {
     ...
     * Example usage:
     * <code>
     * $query->filterByTimeCreated(1234); // WHERE time_created = 1234
     * $query->filterByTimeCreated(array(12, 34)); // WHERE time_created IN (12, 34)
     * $query->filterByTimeCreated(array('min' => 12)); // WHERE time_created > 12
     * </code>
     *
    ...
    public function filterByTimeCreated($timeCreated = null, $comparison = null)
    {
        if (is_array($timeCreated)) {
            ...
            if (isset($timeCreated['min'])) {
                $this->addUsingAlias(UsersTableMap::COL_TIME_CREATED, $timeCreated['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($timeCreated['max'])) {
                $this->addUsingAlias(UsersTableMap::COL_TIME_CREATED, $timeCreated['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(UsersTableMap::COL_TIME_CREATED, $timeCreated, $comparison);
    }
    ...
    /**
     * Filter the query by a related \UserCustomFields object
     *
     * @param \UserCustomFields|ObjectCollection $userCustomFields  the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildUsersQuery The current query, for fluid interface
     */
    public function filterByUserCustomFields($userCustomFields, $comparison = null)
    {
        if ($userCustomFields instanceof \UserCustomFields) {
            return $this
                ->addUsingAlias(UsersTableMap::COL_ID, $userCustomFields->getUserId(), $comparison);
        } elseif ($userCustomFields instanceof ObjectCollection) {
            return $this
                ->useUserCustomFieldsQuery()
                ->filterByPrimaryKeys($userCustomFields->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByUserCustomFields() only accepts arguments of type \UserCustomFields or Collection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the UserCustomFields relation
     *
     ...
     * @return $this|ChildUsersQuery The current query, for fluid interface
     */
    public function joinUserCustomFields($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('UserCustomFields');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'UserCustomFields');
        }
    ...
    /**
     * Use the UserCustomFields relation UserCustomFields object
     *
     * @see useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \UserCustomFieldsQuery A secondary query class using the current class as primary query
     */
    public function useUserCustomFieldsQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinUserCustomFields($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'UserCustomFields', '\UserCustomFieldsQuery');
    }
